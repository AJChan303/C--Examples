...
namespace WindowsFormsApplication2 {
    public class PongGame {
        ...
        public static void DrawIt(Graphics Draw) {   //Draws both paddles and ball
        ...
        {                                                       //changes the Y position of the paddle Accordingly
            if (p1movesUp == true) { int z = paddle1.Y <= 0 ? paddle1.Y = 0 : paddle1.Y -= 3; }
            if (p1movesDown == true) { int z = paddle1.Y >= 381 ? paddle1.Y = 381 : paddle1.Y += 3; }
            if (p2movesUp == true) { int z = paddle2.Y <= 0 ? paddle2.Y = 0 : paddle2.Y -= 3; }
            if (p2movesDown == true) { int z = paddle2.Y >= 381 ? paddle2.Y = 381 : paddle2.Y += 3; }
        }
        ...
        {
            if (ball.X < 1) { p2p += 1; Restart(); }
            else if (ball.X > 579) { p1p += 1; Restart(); }
        }
    ...
    public partial class Form1 : Form {
        ...
        private void Form1_KeyDown(object sender, KeyEventArgs e) {
            if (e.KeyData == Keys.S) { PongGame.p1movesDown = true; }
            if (e.KeyData == Keys.W) { PongGame.p1movesUp = true; }
            if (e.KeyData == Keys.L) { PongGame.p2movesDown = true; }
            if (e.KeyData == Keys.P) { PongGame.p2movesUp = true; }
        }

        private void Form1_KeyUp(object sender, KeyEventArgs e) {
            if (e.KeyData == Keys.S) { PongGame.p1movesDown = false; }
            if (e.KeyData == Keys.W) { PongGame.p1movesUp = false; }
            if (e.KeyData == Keys.L) { PongGame.p2movesDown = false; }
            if (e.KeyData == Keys.P) { PongGame.p2movesUp = false; }
        }

        private void timer2_Tick(object sender, EventArgs e) { PongGame.IncreaseSpeed(); }     //Every 3 seconds, this timer will increase Overall speed
    }
}
